...
def create_agendamento(agendamento: AgendamentoCreate, db: Session = Depends(get_session)):
    ...
    quadra = db.query(Quadra).filter(Quadra.id == agendamento.quadra_id).first()
    if not quadra:
...
def list_agendamentos(db: Session = Depends(get_session)):
    return db.query(Agendamento).all()


@router.get("/{agendamento_id}", response_model=AgendamentoOut)
def get_agendamento(agendamento_id: int, db: Session = Depends(get_session)):
...
def delete_agendamento(agendamento_id: int, db: Session = Depends(get_session)):
    agendamento = db.query(Agendamento).filter(Agendamento.id == agendamento_id).first()
    if not agendamento:
        raise HTTPException(status_code=404, detail="Agendamento não encontrado")
